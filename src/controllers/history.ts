...
export const updateHistory: RequestHandler = async (req, res) => {
  ...
  const histories = await History.aggregate([
    ...
    { $unwind: "$all" },
    {
      $match: {
        "all.date": {
          $gte: startOfDay, // will give us the hsitory from the same day
          $lt: endOfDay,
        },
      },
    },
...
export const getRecentlyPlayed: RequestHandler = async (req, res) => {
  ...
  const unwindWithIndex = {
    $unwind: { path: "$histories", includeArrayIndex: "index" }, //using indexes in aggregations can reduce performance, but here is not the case as we only select 10 audios
  };
