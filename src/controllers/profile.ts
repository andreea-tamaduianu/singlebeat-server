...
export const getRecommendedByProfile: RequestHandler = async (req, res) => {
  //if user is not logged in then fetch the most liked audios
  //if user is logged in, check their recent history (not more than 30 days ago) and recommend according to the audios from that category that they listened to
  const user = req.user;
...
export const getAutoGeneratedPlaylist: RequestHandler = async (req, res) => {
  // find 5 playlists
  // 1 mix 20 - 20 audios from the user's previous history
